...
- **v0.4.0** — 魚が自然に泳ぐようになりました: 群れの中の 1 匹だけが、時々
  すっと駆けたり手前で引き返したりします。獲得点数のロジックも見直し——藻より
  上の生き物が住む岩礁ほど海が肥え、稼ぎは岩礁の値段ごとの帯に揃いました。
  配置パネルと HUD が同じ「次の目標」を指すようになり、`terminarium --help`
  も応えるようになりました。
